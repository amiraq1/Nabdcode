...
# ---------------------------------------------------------------------------
# Regression: extract_json_array (Defect #1 — was stranded/unreachable)
# ---------------------------------------------------------------------------

from engine.deep_agent import extract_json_array


def test_extract_json_array_from_fenced_thinking_output():
    """extract_json_array must recover a JSON array from markdown fences + thinking blocks."""
    raw = (
        "Thinking through the plan...\n"
        "```json\n[\"step one\", \"step two\", \"step three\"]\n```\n"
    )
    parsed = extract_json_array(raw)
    assert parsed == ["step one", "step two", "step three"], parsed


def test_extract_json_array_from_bare_array():
    """extract_json_array handles a plain JSON array with no fences."""
    assert extract_json_array('["a", "b"]') == ["a", "b"]


def test_extract_json_array_from_array_inside_wrapped_text():
    """extract_json_array extracts the first [...] even when prose surrounds it."""
    raw = "Here is the plan: [\"do x\", \"do y\"] hope that helps"
    assert extract_json_array(raw) == ["do x", "do y"]


def test_extract_json_array_returns_empty_on_garbage():
    """extract_json_array returns [] for non-JSON / invalid input."""
    assert extract_json_array("no array here") == []
    assert extract_json_array("") == []
    assert extract_json_array(None) == []


def test_plan_node_produces_non_trivial_plan():
    """plan_node must use extract_json_array to build a real plan (regression for #1 NameError)."""
    llm = _stub_llm('```json\n["research topic", "write report", "review"]\n```')
    agent = NativeDeepAgent(
        runtime_state=RuntimeState(session_id="test-p1-plan"),
        llm_client=llm,
    )
    state = DeepAgentState(task="write a report")
    result_state = agent.plan_node(state)
    assert result_state.plan == ["research topic", "write report", "review"], result_state.plan
