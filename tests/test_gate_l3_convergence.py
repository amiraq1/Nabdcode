...
class TestGateL3Convergence(unittest.TestCase):
    ...
    @patch('engine._loop_helpers._prompt_requires_investigation', return_value=False)
    @patch('engine._loop_helpers._has_active_goal', return_value=False)
    @patch('core.convergence_gate.can_finalize')
    def test_no_read_task_does_not_force_three_reads(self, mock_can, mock_goal, mock_inv):
    ...
    @patch('core.convergence_gate.can_finalize')
    def test_read_required_task_enforces_threshold(self, mock_can, mock_goal, mock_inv):
        self.engine._real_reads = MagicMock(return_value=2)
        mock_can.return_value.allowed = True
        
        # Should return False (rejected) because reads < 3
    ...
    @patch('engine._loop_helpers._has_active_goal', return_value=True)
    @patch('core.convergence_gate.can_finalize')
    def test_no_final_answer_before_three_reads_when_policy_requires_it(self, mock_can, mock_goal, mock_inv):
        self.engine._real_reads = MagicMock(return_value=0)
        mock_can.return_value.allowed = True
