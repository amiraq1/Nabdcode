...
def test_systematic_planning_goal_persistence():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        sm.goal = {
            ...
            "rejection_count": 1,
            "goal_text": "Verify Nabd OS persistence",
        }
        sm.save()

        sm2 = SessionManager(Path(td))
        assert sm2.load(sm.session_id)
        assert sm2.goal is not None
        assert sm2.goal["rejection_count"] == 1
        assert sm2.goal["status"] == "active"

        prompt = build_goal_prompt(sm2.goal["goal_text"], kind="bootstrap")
        assert "<goal-bootstrap>" in prompt
        assert "Begin working on this goal:" in prompt
