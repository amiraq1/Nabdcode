    ...
    def dispatch(self, tool_name: str, kwargs: dict, timeout: int = 30) -> ToolResult:
        ...
        try:
            ...
            try:
                ...
                # future.cancel() only cancels tasks that have NOT started yet;
                # a task already running inside the pool worker cannot be
                # interrupted. The semaphore is released below so a queued task
                # can take the slot, but a genuinely hung tool.execute keeps
                # running on its own worker thread until it returns on its own.
                future.cancel()
                return result
